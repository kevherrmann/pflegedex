...
export default function Show({ resident, carePlan, sisStatus, canEdit, topics, latestGeneration }: Props) {
    ...
    useEffect(() => {
        ...
        const handle = setInterval(async () => {
            try {
                const url = route('residents.care-plan.generate.show', [resident.id, generation.id]);
    ...
    const handleEvaluate = () => {
        if (!confirm('Maßnahmenplan-Evaluation jetzt speichern? Der nächste Termin wird auf +8 Wochen gesetzt.')) {
            return;
        }
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-[#333333]">Maßnahmenplan · {resident.formalName}</h2>}
        >
                ...
                <div className="mx-auto max-w-5xl space-y-6 sm:px-6 lg:px-8">
                    ...
                    {justFailed && generation !== null && (
                        <div className="rounded-2xl border border-red-300 bg-red-50 p-6 shadow-sm">
                            <p className="text-sm font-bold uppercase tracking-widest text-red-800">KI-Generierung fehlgeschlagen</p>
                            <p className="mt-1 text-xs text-red-900">{generation.errorMessage ?? 'Unbekannter Fehler. Bitte erneut versuchen.'}</p>
                            <p className="mt-1 text-xs text-red-900">
                                Der Maßnahmenplan ist weiterhin angelegt — Sie können den Versuch über „Erneut generieren" wiederholen oder den MP manuell bearbeiten.
                            </p>
                        </div>
                    )}
                    ...
                    {canEdit && !aiAvailable && !isRunning && (
                        <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 shadow-sm">
                            <p className="text-xs font-semibold uppercase tracking-widest text-amber-900">KI-Funktion nicht verfügbar</p>
                            <p className="mt-1 text-xs text-amber-900">
                                {aiStatus?.reason ?? 'Ollama-Service oder Modell ist gerade nicht erreichbar.'} Bearbeiten und Speichern funktioniert weiterhin normal.
                            </p>
                        </div>
                    )}
                    ...
                    {!sisStatus.exists && (
                        <div className="rounded-2xl bg-amber-50 p-6 ring-1 ring-amber-200">
                            ...
                            <p className="mt-1 text-sm text-amber-800">
                                Für diesen Bewohner ist noch keine SIS angelegt. Der Maßnahmenplan kann erst nach Fertigstellung der SIS erstellt werden.
                            </p>
                            <Link href={route('residents.sis.show', resident.id)} className="mt-3 inline-block text-sm font-semibold text-amber-900 underline">
                                Zur SIS
                            </Link>
                        </div>
                    )}
                    {sisStatus.exists && !sisStatus.completed && (
                        <div className="rounded-2xl bg-amber-50 p-6 ring-1 ring-amber-200">
                            <p className="text-sm font-semibold text-amber-900">SIS noch nicht fertiggestellt</p>
                            <p className="mt-1 text-sm text-amber-800">
                                Der Maßnahmenplan kann erst angelegt werden, wenn die SIS fachlich fertiggestellt wurde (Button „Fertigstellen" auf der SIS-Seite).
                            </p>
                            <Link href={route('residents.sis.show', resident.id)} className="mt-3 inline-block text-sm font-semibold text-amber-900 underline">
                                Zur SIS
                            </Link>
                    ...
                    {carePlan === null
                        ? sisStatus.completed && (
                              <div className="rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-[#E5E7EB]">
                                  <p className="text-gray-700">Für diesen Bewohner ist noch kein Maßnahmenplan angelegt.</p>
                                  <p className="mt-1 text-xs text-gray-500">SIS fertiggestellt am {sisStatus.completedAt ?? '—'}.</p>
                                  {canEdit && (
                                      <p className="mt-3 text-xs text-gray-500">
                                          Über die SIS-Seite kann ein Maßnahmenplan per Button „MP generieren" aus der SIS erstellt werden.
                                      </p>
                                  )}
                              </div>
                          )
                              ...
                              <>
                                  <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-[#E5E7EB]">
                                      <div className="flex flex-wrap items-start justify-between gap-4">
                                          <div>
                                              <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#9B1C3B]">Maßnahmenplan</p>
                                              <p className="mt-2 text-sm text-gray-600">
                                                  Begonnen: {carePlan.startedAt ?? '—'} · Letzte Evaluation: {carePlan.evaluatedAt ?? 'noch keine'}
                                              </p>
                                              <p className="mt-1 text-sm">
                                                  Nächste Evaluation:{' '}
                                                  {carePlan.nextEvaluationDue ? (
                                                      <span className={carePlan.isOverdue ? 'font-semibold text-red-700' : 'text-gray-700'}>
                                                          {carePlan.nextEvaluationDue}
                                              ...
                                              <p className="mt-1 text-xs text-gray-500">{carePlan.versionCount} Version(en) im Archiv.</p>
                                          </div>
                                          {canEdit && (
                                              <div className="flex flex-wrap gap-2">
                                                  <Link
                                                      href={route('residents.care-plan.edit', resident.id)}
                                                  ...
                                                  {!isRunning && (
                                                      <button
                                                          ...
                                                          title={aiAvailable ? '' : 'KI ist nicht verfügbar'}
                                                      >
                                                          Erneut generieren
                                                      </button>
                                                  )}
                                      ...
                                      {carePlan.grundbotschaft && (
                                          <div className="mt-6 rounded-md bg-[#FAE7EC]/30 p-4">
                                              <p className="text-xs font-bold uppercase tracking-widest text-[#9B1C3B]">Grundbotschaft</p>
                                              <p className="mt-1 whitespace-pre-line text-sm text-gray-800">{carePlan.grundbotschaft}</p>
                                          </div>
                                      )}
                                  </div>
                                  ...
                                  <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-[#E5E7EB]">
                                      ...
                                      {carePlan.topics.length === 0 ? (
                                          <p className="mt-4 text-sm text-gray-500">
                                              Noch keine Themenblöcke befüllt. Über „Bearbeiten" können relevante Themen ergänzt werden.
                                          </p>
                                      ) : (
                                              ...
                                              {carePlan.topics.map((t) => (
                                                  <div key={t.topicNumber}>
                                                      <p className="text-sm font-semibold text-gray-800">
                                                          {t.topicNumber}. {labelByNumber.get(t.topicNumber) ?? '—'}
                                                      </p>
                                                      <p className="mt-1 whitespace-pre-line text-sm text-gray-700">{t.content}</p>
                                                  </div>
                                              ))}
                                          </div>
                                      )}
